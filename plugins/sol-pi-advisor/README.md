...
`pi_lane_batch_start` creates the wave; `pi_lane_batch_drive` waits for all lanes or
aborts those still active. Corrections remain lane-specific through
`pi_lane_drive`. Shared-file and dependent work is intentionally rejected from the
parallel path and must run in a later serial/DAG wave.
